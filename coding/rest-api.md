...
- **`message`는 개발자용.** 문서를 찾지 않아도 원인을 파악할 수 있게
  ...
  보고 결정한다 (i18n 매핑).
- **`result` 안에서 데이터는 리소스 이름을 키로 담는다.** `result`를
  익명 컨테이너로 쓰지 않는다 — 무엇이 담겼는지 키 이름으로 드러나고,
  여러 리소스를 나란히 담을 수도 있다.
  - 단일 조회: 단수 키 — `"result": {"user": {...}}`
  - 목록 조회: 복수 키 — `"result": {"users": [...], "page": 2, ...}`
    (페이지네이션 메타는 리소스 키 옆에 나란히)
  - 실패 시에는 `null`. 검증 오류처럼 세부 정보가 있으면 그 구조를 담을
    수 있다 (`{"invalid_fields": [...]}` 등).
