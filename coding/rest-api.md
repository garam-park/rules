...
- **의미대로 사용한다.** 성공은 2xx, 클라이언트 오류는 4xx, 서버 오류는
  5xx. "항상 200 + 본문 코드" 방식은 쓰지 않는다 — 모니터링·캐싱·HTTP
  클라이언트 등 생태계 전체가 상태 코드를 전제로 동작하기 때문.
- 세부 구분은 본문의 에러 코드가 담당하고, 상태 코드는 큰 분류만 맡는다.
...
- **날짜·시각은 ISO 8601, UTC**로 반환한다 — `"2026-07-11T03:24:00Z"`.
  타임존 변환은 표시하는 쪽(클라이언트)의 책임.
- **null 필드는 생략하지 않고 포함한다.** 필드 존재 여부가 상황마다
  흔들리면 클라이언트 파싱 코드가 방어적으로 변한다. 스키마는 항상 같은
  모양을 유지한다.
