...
| `create` | 새로 만들어 저장 |
| `update` | 기존 것을 수정 |
| `save` | 있으면 수정, 없으면 생성 (upsert). 호출부가 존재 여부를 몰라도 된다 |
| `delete` | 삭제 |
...
쓰기 동사는 **호출부가 존재 여부를 아는지**로 가른다.

- **존재 여부를 호출부가 알고 있으면** `create`/`update`를 쓴다. 이름이 더
  구체적이고, 잘못된 상황(중복 생성, 없는 것 수정)을 계약으로 막을 수 있다.
- **존재 여부와 무관하게 "이 상태로 만들어라"가 의도면** `save`를 쓴다.
  로컬 저장소·설정·파일 쓰기처럼 멱등하게 덮어쓰는 경우가 여기에 해당한다.
  - `saveSettings(settings)`, `saveDraft(draft)`
- `save`는 결과를 반환할 때 저장된 최종 상태를 돌려준다.
- 존재 여부를 모르는 함수에 `create`/`update` 중 하나를 억지로 고르면
  이름의 절반이 틀린다. `save`는 그 절반을 잃지 않기 위한 동사다.
